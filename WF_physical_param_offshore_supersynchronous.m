...
% run('initialize_WT_default_100625.m');
% run('initialize_WT_default_180625.m');
% run('initialize_WT_default_gfm.m');
run('initialize_WT_default_offshore_super.m');
...
% Z_wf_cable_g4_ser=Z_wf+((((Z_C_C^-1+(Z_line_G*(0.8))^-1+(Z_comp)^-1)^-1+Z_line_C*(1))^-1+Z_C_C^-1+(Z_comp*0.8)^-1)^-1);
% Z_wf_cable_g4_ser=(((((Z_C_C/rat)^-1+(Z_line_G*rat)^-1+(Z_comp*rat)^-1)^-1+Z_line_C*rat)^-1+(Z_C_C/rat)^-1+(Z_comp*rat)^-1)^-1);
Z_grid_ser=Z_line_G/3+(((Z_wf^-1+Z_C_C^-1+Z_comp^-1)^-1+Z_line_C)^-1+Z_C_C^-1+Z_comp^-1)^-1;
...
Z_eq=Z_line_G;
[~,n_f] = min(abs(Z_eq.Frequency-f1));
Z_f1=Z_eq.ResponseData(:,:,n_f);
r_g_agg=(r_tf_wt+r_tf_wf+real(Z_f1(1,1)/Z_b_G));
l_g_agg=(l_tf_wt+l_tf_wf+imag(Z_f1(1,1)/Z_b_G)/(2*pi*f1));
run('initialize_WT_default_agg.m');
[Z_wf,Z_conv,cvtr_agg,Z_wt]=IM_WF_agg(freq,Sb_WF,cvtr_agg,[r_tf_wt,l_tf_wt;r_tf_wf,l_tf_wf],V_coll);
Z_wf=Z_wf*(V_trans/V_coll)^2;
Z_wf_cable_g4_ser=Z_wf+(((Z_C_C^-1+Z_line_G^-1+Z_comp^-1)^-1+Z_line_C)^-1+Z_C_C^-1+Z_comp^-1)^-1;
Z_wf_cable_g4=(Z_wf^-1+((Z_C_C^-1+Z_line_G^-1+Z_comp^-1)^-1+Z_line_C)^-1+Z_C_C^-1+Z_comp^-1)^-1;
% Z_wf_OH_ser=Z_wf+(Z_line_OH+Z_line_G)/3;
Z2=Z_wf_cable_g4;
Lg1=Lg;
cvtr=cvtr_agg;
...
% ylim([-750 750])
line([Z.Frequency(1) Z.Frequency(end)],[0 0],'Color','k');
legend('$L_g=1$ pu','$L_g=2$ pu','interpreter','latex','Location','southeast');


% exportgraphics(f,[path_for_save,'Z_offshore_case','.pdf']);
...
Lg=0.1*((Z_b_G/(2*pi*f1)));
Z_line_G=IM_line(freq,Rg,Lg);   
Z_line_C=IM_line(freq,r_c_eff*D_C2,l_c_eff*D_C2);    %Cable series imp
Z_C_C=IM_C(freq,c_c_eff/2*D_C2);                  %Cable shunt imp
l_comp=(1/(c_c_eff/2*D_C2*2*pi*f1))/Z_b_G-(l_tf_wf+(l_c_eff*D_C2*2*pi*f1)/Z_b_G);
L_comp=sqrt(2)*l_comp*(Z_b_G/(2*pi*f1));
R_comp=0.1*l_comp*Z_b_G;
Z_comp = IM_line(freq,R_comp,L_comp); 
...
im_part2=imag(Z2.ResponseData);
mag2=abs(Z2.ResponseData);
% p2=semilogx(fout,squeeze(im_part2(o,i,:)),'Color',[0.9290 0.6940 0.1250]);
p2=loglog(fout,squeeze(mag2(o,i,:)),'Color',[0.9290 0.6940 0.1250]);
p2.LineWidth=1.5;

im_part3=imag(Z3.ResponseData);
mag3=abs(Z3.ResponseData);
% p3=semilogx(fout,squeeze(im_part3(o,i,:)),'Color','#9C35B6');
p3=loglog(fout,squeeze(mag3(o,i,:)),'Color','#9C35B6');
p3.LineWidth=1.5;

% ylim([-750 750])
line([Z.Frequency(1) Z.Frequency(end)],[0 0],'Color','k');
line([neg_pass_freq neg_pass_freq],[ax.YLim(1) ax.YLim(end)],'Color','k','LineStyle','--');
legend(['$L_g=0.1$ pu, ', num2str(D_C),' km'],'$L_g=0.2$ pu, 50 km',['$L_g=0.1$ pu, ', num2str(D_C2),' km'],'interpreter','latex','Location','southeast');


% exportgraphics(f,[path_for_save,'Z_offshore_super','.pdf']);
...
p2.LineWidth=1.5;
xlabel('Frequency (Hz)','interpreter','latex');ylabel('$\Im\{Z_{on,pp}(j\omega)\}$','Interpreter','latex');
ax=gca;
% ylim([1 ax.YLim(2)])
f_size=16;
ax.FontSize=f_size;
...
cvtr.parameters.model_selector=3;
run('initialize_params_agg.m');
load('vals_pn_adm_WF_060525_qsem');
...
for i=1:length(p_vary)
    ...
    nexttile(t,1);
    p2=semilogx(fout,pass,'Color',color_plot(i,:));
    ...
    nexttile(t,2);
    % p3=loglog(fout,squeeze(mag(out,in,:)),'Color',color_plot(i,:));
...
t=tiledlayout(3,1);
t.TileSpacing='compact';
f.Position=f.Position.*[0,0,0.75*3,1.5];
...
exportgraphics(f,[path_for_save,'vary_rg','.pdf']);
