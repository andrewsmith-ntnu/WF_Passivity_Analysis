function conv = steadystate_VSM_DEM(conv,input)
...
% sol_ss=fsolve(@(x)steadystate_nonlinear_VSM_DEM(x,conv),[1,0,0]);
sol_ss=fsolve(@(x)steadystate_nonlinear_VSM_DEM(x,conv),[-0.01,conv.ss.pref0,0],fsolve_options);
% sol_ss
