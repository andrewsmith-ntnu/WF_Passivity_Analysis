function [Z_wf,Z_wt_GFL,cvtr_GFL,cvtr_GFM]=IM_WF3(freq,Sb_WF,Sb_WT,n_GFL,n_GFM,cvtr,D_WW,cable_params,tf_params,V_coll)
...
if ~isinteger(int8(Sb_WF/Sb_WT))
    error('Not a integer number of turbines')
end
...
cvtr_GFL.parameters.model_selector=6;
cvtr_GFM.parameters.model_selector=3;

[Z_wt_GFL,~,cvtr_GFL]=IM_WT_all(freq,cvtr_GFL);
Z_wt_GFL=Z_wt_GFL*cvtr_GFL.pu.Zb*(V_coll/cvtr_GFL.parameters.Vnr)^2;

[Z_wt_GFM,~,cvtr_GFM]=IM_WT_all(freq,cvtr_GFM);
Z_wt_GFM=Z_wt_GFM*cvtr_GFM.pu.Zb*(V_coll/cvtr_GFM.parameters.Vnr)^2;

Z_WT_T_GFL=Z_wt_GFL+Z_trans_WT;
Z_s_GFL=Z_WT_T_GFL;

Z_WT_T_GFM=Z_wt_GFM+Z_trans_WT;
Z_s_GFM=Z_WT_T_GFM;

Z_wf=(n_GFL*Z_s_GFL^-1+n_GFM*Z_s_GFM^-1)^-1;
