...
colorIdx = colorIdx + 1;
fout = Z1.Frequency;
p = semilogx(fout, pass, 'Color', color);
hold on
ylabel('Passivity');
grid on;
p.LineWidth = 1.5;
...
nexttile(t,2);
if exist('plot_type', 'var') && strcmp(plot_type, 'imag')
    p = semilogx(fout, squeeze(im_part(n_o,n_i,:)), 'Color', color);
    ylabel(['$Im[Z_{', Z2.InputName{n_i}, '-', Z2.OutputName{n_o}, '}]$'], 'Interpreter', 'latex');
    hold on
else
    p = loglog(fout, squeeze(mag(n_o,n_i,:)), 'Color', color);
